...
export default function Header2({ darkMode, setDarkMode }) {
  ...
  const navigate = useNavigate();
  const location = useLocation();
  const isDark = theme === "dark" || darkMode;
      ...
      >
        ...
        >
          <div className="flex h-16 items-center justify-between px-4 sm:px-6 lg:px-8">
            ...
            <nav className="hidden items-center gap-3 lg:flex">
              {navItems.map((item) => {
                const isActive = isNavActive(location.pathname, location.hash, item.href);
                const isHovered = hoveredItem === item.name;
                const showIndicator = isActive || isHovered;
                  ...
                  >
                    ...
                    >
                      {showIndicator && (
                        <motion.div
                          className={`absolute inset-0 rounded-full ${
                            isActive ? "bg-indigo-500/15 ring-1 ring-indigo-500/30" : "bg-muted/80"
                          }`}
            ...
            <motion.div className="hidden items-center space-x-3 lg:flex" variants={itemVariants}>
              ...
              {user ? (
                ...
                <>
                  <Link to="/login" className="text-foreground/80 hover:text-foreground px-4 py-2 text-sm font-medium transition-colors duration-200">
                    Sign In
                  </Link>
                  <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
                    <Link to="/signup" className="bg-foreground text-background hover:bg-foreground/90 inline-flex items-center space-x-2 rounded-lg px-5 py-2.5 text-sm font-medium shadow-sm transition-all duration-200">
                      <span>Get Started</span>
                      <ArrowRight className="h-4 w-4" />
                    </Link>
                  </motion.div>
