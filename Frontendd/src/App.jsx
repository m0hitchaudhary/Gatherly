...
// Protected Route Component (Updated for case-insensitive role checking)
const ProtectedRoute = ({ children, allowedRoles }) => {
  const { user, loading } = useAuth();
  ...
  if (allowedRoles) {
    const userRole = user.role?.toLowerCase() || 'attendee';
    const normalizedAllowedRoles = allowedRoles.map(r => r.toLowerCase());
    
    if (!normalizedAllowedRoles.includes(userRole)) {
      console.warn(`Redirecting: User role '${userRole}' not in allowed roles.`, normalizedAllowedRoles);
      return <Navigate to="/" replace />;
    }
  }
