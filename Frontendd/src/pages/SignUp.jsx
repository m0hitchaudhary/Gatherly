...
export default function SignUp() {
    ...
    const validateField = (name, value) => {
        switch (name) {
            ...
            case 'confirmPassword':
                return value !== formData.password ? 'Passwords do not match' : '';
            case 'role':
                return !['attendee', 'organizer', 'admin'].includes(value) ? 'Please select a valid role' : '';
            default:
                return '';
        }
    };
    ...
    const isFormValid =
        ...
        formData.password.length >= 6 &&
        formData.confirmPassword === formData.password &&
        ['attendee', 'organizer', 'admin'].includes(formData.role) &&
        agreeTerms;
                ...
                >
                    ...
                    >
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6 relative z-10">
                            ...
                            <div className="space-y-2 group">
                                <label className="text-xs font-medium text-gray-600 group-focus-within:text-[#e63946] transition-colors uppercase tracking-wider" htmlFor="role">
                                ...
                                <select
                                    ...
                                    value={formData.role}
                                    onChange={handleChange}
                                    className="w-full bg-white/50 border border-white/50 rounded-lg px-4 py-3 text-gray-900 focus:outline-none focus:ring-2 focus:ring-[#e63946] focus:border-transparent transition-all duration-300 text-base backdrop-blur-sm shadow-sm cursor-pointer"
                                >
                                    <option value="attendee">Attendee / Customer</option>
